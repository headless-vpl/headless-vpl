...
対応 API: `DragAndDrop`, `NestingZone`, `SnapConnection`
...
- **Primitive**: `headless-vpl/primitives`。`Workspace`, `Container`, `Connector`, `Edge`, `AutoLayout`, `SvgRenderer` などの低レベル building blocks
- **Helper**: `headless-vpl/helpers`。`SnapConnection`, `NestingZone`, `EdgeBuilder`, `DomSyncHelper`, `bindWheelZoom` などの任意 helper
- **Recipe**: `headless-vpl/recipes`。`InteractionManager`, `bindDefaultShortcuts` のような統合 convenience layer

| モジュール | 役割 |
|---|---|
| **Renderer** (`SvgRenderer`) | デバッグ用ワイヤーフレーム描画、仮想座標の計算 |
| **Controller** (`InteractionManager`) | Recipe 層の統合 orchestrator。DnD、パン、マーキー、リサイズ、Edge 作成をまとめる |
...
├── util/                          # ユーティリティ関数
│   ├── interaction.ts             # InteractionManager（recipe 層 orchestrator）
│   ├── interactionTargets.ts      # ポインタ hit-test と intent 解決
│   ├── interactionArbitration.ts  # snap / nesting 優先順位の調停
│   ├── dnd.ts                     # ドラッグ&ドロップ
...
├── blocks/                        # ブロック型 VPL ヘルパー
│   ├── stack.ts                   # BlockStackController
│   ├── connect.ts                 # スタック接続ヘルパー
│   ├── dom.ts                     # DOM サイズ監視
│   └── zones.ts                   # 挿入ゾーン（内部利用）
├── primitives.ts                  # primitive 層 entrypoint
├── helpers.ts                     # helper 層 entrypoint
├── recipes.ts                     # recipe 層 entrypoint
├── blocks.ts                      # block helper entrypoint
└── index.ts                       # 互換維持用のフラット export
```
